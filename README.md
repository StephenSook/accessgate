...
| **Granite Embedding (watsonx.ai)** | The same citation retrieval on the hosted demo, where the local embedding stack is absent. Called over REST, so it needs no torch | Wired, hosted (`src/watsonx_embedding.py`, `ibm/granite-embedding-278m-multilingual`) |
