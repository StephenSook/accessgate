...
def _citation_provenance() -> dict:
    ...
    try:
        ...
        return {
            ...
            "note": "Granite Embedding r2 on a local install; the deterministic character n-gram encoder on this hosted deploy, deliberately, to keep per-cold-start re-embedding from exhausting the metered token quota. Whatever is reported in `active` is what actually served the citations you are reading.",
        }
    except Exception as exc:  # noqa: BLE001
    ...
    return JSONResponse(content={
        ...
        "tiers": {
            "wired_live": [
                ...
                {"name": "RAG citation engine", "evidence": "src/rag.py", "note": "Citations are retrieved at runtime rather than mapped rule-to-string. The corpus is the six Docling-parsed standard pages (206 of 218 chunks) plus committed short-form clause text for the same six standards (12 chunks, src/rag.py _INLINE_STANDARDS). A local install embeds it with Granite Embedding r2. THIS HOSTED DEPLOY retrieves with the deterministic character n-gram encoder, on purpose: re-embedding 218 chunks through a metered API on every cold start, on an instance with an ephemeral filesystem that cold-starts about twenty times a day, exhausts the plan token quota in under a day (it did, on 2026-07-27, taking the vision and Guardian calls down with it). See citation_provenance below for the encoder actually serving this instance right now. The corpus and the citations are identical either way; only the retrieval encoder differs."},
            ...
            "integration": [
                ...
                {"name": "Granite Embedding (watsonx.ai)", "evidence": "src/watsonx_embedding.py", "note": "Model id ibm/granite-embedding-278m-multilingual over REST, needing no torch. Genuinely wired and exercised, but NOT run on this hosted deploy: it is reserved for building a publishable index offline, behind ACCESSGATE_ALLOW_HOSTED_REINDEX=1, because per-cold-start re-embedding exhausts the token quota. This is a deliberate cost decision, not a broken integration."},
